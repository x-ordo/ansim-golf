...
import { TeeTime } from './types'; // Removed MOCK_TEE_TIMES
import { useTeeTimes } from './src/hooks/useTeeTimes'; // Corrected Path

const App: React.FC = () => {
  const { data: teeTimes, loading, error } = useTeeTimes(); // Hook Integration
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f4f7fa]">
        <div className="text-center p-10 bg-white rounded-3xl shadow-lg border border-red-100">
          <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6 text-red-500 text-xl">!</div>
          <h3 className="text-xl font-black text-slate-900 mb-3">데이터를 불러올 수 없습니다</h3>
          <p className="text-base text-slate-500 mb-8">{error}</p>
          <button onClick={() => window.location.reload()} className="px-8 py-3 bg-slate-900 text-white rounded-2xl text-base font-bold">다시 시도</button>
        </div>
      </div>
    );
  }
          ...
          <main className="flex-1 max-w-7xl mx-auto w-full">
            ...
            <div className="sticky top-14 z-30 bg-white/95 backdrop-blur-md px-6 py-6 border-b border-slate-200/60 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => setIsLoginOpen(true)} className="text-xs font-bold text-slate-400 hover:text-emerald-600 transition-colors">로그인</button>
            ...
            <div className="px-0 md:px-8 py-0 md:py-10">
              <div className="flex flex-col md:flex-row gap-12">
                ...
                <aside className="hidden md:block w-72 space-y-12 py-2">
                  <div>
                    <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-8">Region Filter</h2>
                  ...
                  {viewMode === 'PRO' && (
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 px-6 md:px-0">
                      ...
                      <div className="relative w-full md:w-80">
                        ...
                        />
                        <svg className="absolute left-5 top-3.5 w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  {filteredTeeTimes.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-32 bg-white rounded-[40px] border border-slate-100 mx-4 md:mx-0 shadow-sm">
                      <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mb-6">
                        <svg className="w-10 h-10 text-slate-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 9.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      </div>
                      <h3 className="text-lg font-bold text-slate-900 mb-2">검색 결과가 없습니다</h3>
                      <p className="text-slate-400 text-sm font-medium">다른 지역이나 날짜를 선택해 보세요.</p>
                    </div>
                  )}
                </section>
              </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-16 mb-16">
            <div className="max-w-xs">
              <div className="flex items-center gap-3 mb-8">
                ...
                <span className="text-2xl font-black text-slate-900 tracking-tight">안심<span className="text-emerald-600">골프</span></span>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-16 text-xs font-bold">
              <div>
                ...
                <ul className="space-y-4 text-slate-500 font-medium">
                  ...
                  <li><button onClick={() => setViewMode('SAAS')} className="hover:text-blue-600 text-xs bg-blue-50 px-3 py-1.5 rounded-lg">파트너 전용</button></li>
                </ul>
              </div>
              <div>
